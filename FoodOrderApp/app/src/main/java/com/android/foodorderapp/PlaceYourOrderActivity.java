...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    private void calculateTotalAmount(RestaurantModel restaurantModel) {
        ...
        tvSubtotalAmount.setText("đ"+String.format("%.3f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("đ"+String.format("%.3f", restaurantModel.getDelivery_charge()));
            subTotalAmount += restaurantModel.getDelivery_charge();
        }
        tvTotalAmount.setText("đ"+String.format("%.3f", subTotalAmount));
    }

    private void onPlaceOrderButtonClick(RestaurantModel restaurantModel) {
        if(TextUtils.isEmpty(inputName.getText().toString())) {
            inputName.setError("Hãy nhập tên khách hàng ");
            return;
        }else if(TextUtils.isEmpty(inputPhone.getText().toString())){
            inputPhone.setError("Hãy nhập số điện thoại ");
            return;
        } else if(isDeliveryOn && TextUtils.isEmpty(inputAddress.getText().toString())) {
            inputAddress.setError("Hãy nhập địa chỉ ");
            return;
        }
